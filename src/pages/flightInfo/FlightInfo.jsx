...
// import { returnData as flightData } from "./returndata";
...
const FlightInfo = () => {
  ...
  return (
    <>
    {flightData.length>0 ? <section className="py-5">
            ...
            <ul className="flex items-center text-center gap-4 bg-slate-100 rounded-3xl py-1 text-sm sticky top-28 cursor-pointer z-50 font-poppins font-light">
              ...
              {criteriaList.map((criteria, ind) => (
                <li
                  key={ind}
                  className={`${
                    criteria.key === "duration" ? "basis-[12%]" : "basis-[16%]"
                  } ${
              ...
              <li onClick={handleClearSort} className="text-xs text-blue-500 basis-[12%]">Clear all</li>
            </ul>
            {flightData.map((flightData, index) => {
              const { dictionaries } = flightData;
              return flights && flights.length > 0 ? (
                flights.map((flight, num) => {
                  const [itineraries, returnItineraries] = flight?.itineraries;
                  return (
                    <div key={`${index}-${num}`}>
